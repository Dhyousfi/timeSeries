...
def zip_dir(fzip, dirpath, compress_type):
    # code from Dmitry Nedbaylo https://stackoverflow.com/questions/458436/adding-folders-to-a-zip-file-using-python
    ...
    for root, dirs, files in os.walk(dirpath):
        ...
        for f in files:
            if f[-1] == '~' or (f[0] == '.' and f != '.htaccess'):
                ...
                continue
            fzip.write(root + '/' + f, dirname + '/' + f, compress_type)
...
class Bundle:
	def __init__(self, starting_kit_dir, big_data_dir):
		...
		self.sample_result_submission = os.path.join(starting_kit_dir, 'sample_results')  # sample_result_submission
	...
	def check(self):
		...
		results = ls(os.path.join(starting_kit_dir, '*/*.pickle'))
		if len(results)!=1: 
			print("[-] Failed, prediction file is missing from {}:".format(starting_kit_dir))
			return 0
		else:
			print("[+] Passed")
		print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
